...
namespace Microsoft.Azure.WebJobs.Script.WebHost.Management
{
    public class FunctionsSyncManager : IFunctionsSyncManager, IDisposable
    {
        ...
        public async Task<SyncTriggersResult> TrySyncTriggersAsync(bool checkHash = false)
        {
            try
            {
                ...
                if (checkHash)
                {
                    ...
                    (shouldSyncTriggers, hash) = await CheckHashAsync(json);
                }
                ...
                if (shouldSyncTriggers)
                {
                    ...
                    {
                        // if we have an updated hash, store it
                        await UpdateHashAsync(hash);
                    }
        ...
        internal async Task<(bool, string)> CheckHashAsync(string content)
        {
            try
            {
            ...
            // return false if we don't have storage access or
            // if any other failures occurr
            return (false, null);
        }

        internal async Task UpdateHashAsync(string hash)
        {
            try
            {
                var hashBlob = await GetHashBlobAsync();
